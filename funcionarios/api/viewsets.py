...
from ..models import Email, Employees, Offers
...
def get_offers(request):
    ...
    serializer = serializers.OffersSerializer(instance=offers, many=True)
    return Response(serializer.data, status=status.HTTP_200_OK)


@swagger_auto_schema(method='POST', request_body=serializers.EmailSerializer)
@api_view(http_method_names=['POST'])
def patient_message(request):
    ...
    serializer = serializers.EmailSerializer(instance=emails, many=True)
    list = []
    for row in serializer.data:
            list.append(dict(row))
    contador = 0
    for i in list:
        if(id_request == i['id_request']):
            contador = contador + 1
    if(contador == 0):
            corpo_email = f"""
            <p>Olá, {nome}</p>
